"use client";

import Image from "next/image";
import Link from "next/link";
import { useEffect, useState } from "react";
import AdminHeader from "@/components/admin/AdminHeader";
import AdminPublishBadge from "@/components/admin/AdminPublishBadge";
import { useAuth } from "@/components/AuthProvider";
import { subscribeReferences, updateReference } from "@/lib/firestore-content";
import type { ReferenceItem } from "@/lib/types";

export default function AdminReferencesPage() {
  const { user, firebaseReady } = useAuth();
  const [refs, setRefs] = useState<ReferenceItem[] | null>(null);
  const [togglingId, setTogglingId] = useState<string | null>(null);

  useEffect(() => {
    if (!firebaseReady || !user) return;
    const unsub = subscribeReferences((rows) => setRefs(rows));
    return () => unsub();
  }, [firebaseReady, user]);

  async function handleTogglePublished(r: ReferenceItem) {
    setTogglingId(r.id);
    try {
      await updateReference(r.id, { published: !(r.published !== false) });
    } catch (err) {
      console.error(err);
    } finally {
      setTogglingId(null);
    }
  }

  return (
    <>
      <AdminHeader
        eyebrow="Références"
        title="Références & presse"
        description="Logos, articles et éléments de référence affichés sur le site."
        action={{
          href: "/admin/references/nouveau",
          label: "+ Nouvelle référence",
        }}
      />

      {refs === null ? (
        <p className="text-sm text-neutral-500">Chargement…</p>
      ) : refs.length === 0 ? (
        <div className="border border-dashed border-black/20 bg-white p-12 text-center">
          <p className="font-display text-2xl">Aucune référence</p>
          <p className="mt-3 text-sm text-neutral-600">
            Ajoutez un logo ou une mention presse.
          </p>
          <Link
            href="/admin/references/nouveau"
            className="btn-line mt-6 inline-flex"
          >
            + Nouvelle référence
          </Link>
        </div>
      ) : (
        <ul className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {refs.map((r) => (
            <li key={r.id}>
              <div className="border border-black/10 bg-white transition-colors hover:bg-neutral-50">
                <Link
                  href={`/admin/references/edit?id=${r.id}`}
                  className="group block"
                >
                  <div className="relative aspect-square w-full overflow-hidden bg-neutral-100 p-6">
                    {r.imageUrl && (
                      <Image
                        src={r.imageUrl}
                        alt={r.title}
                        fill
                        sizes="(min-width: 1024px) 33vw, (min-width: 640px) 50vw, 100vw"
                        className="object-contain p-4"
                        unoptimized
                      />
                    )}
                    <div className="absolute right-2 top-2 z-10">
                      <AdminPublishBadge published={r.published} />
                    </div>
                  </div>
                  <div className="p-4">
                    <h3 className="font-display text-xl leading-tight">
                      {r.title}
                    </h3>
                    {r.description && (
                      <p className="mt-1 line-clamp-2 text-sm text-neutral-600">
                        {r.description}
                      </p>
                    )}
                  </div>
                </Link>
                <div className="flex items-center justify-end gap-3 border-t border-black/10 px-4 py-3">
                  <button
                    type="button"
                    disabled={togglingId === r.id}
                    onClick={() => handleTogglePublished(r)}
                    className="text-xs uppercase tracking-wide-xl text-neutral-700 underline-offset-2 hover:text-black hover:underline disabled:opacity-50"
                  >
                    {togglingId === r.id
                      ? "…"
                      : r.published === false
                      ? "Publier"
                      : "Masquer"}
                  </button>
                </div>
              </div>
            </li>
          ))}
        </ul>
      )}
    </>
  );
}
